...
const NFTView = ({wallet}) => {
    ...
    const getNfts = async() => {
        ...
        setLoading(true)
        try {
            let response = await axios.get(`https://eth-mainnet.g.alchemy.com/v2/5gLsN5LmEaadZhjV5fGwt1wDs-7inufk/getNFTs?owner=${wallet}&withMetadata=true`)
            ...
            while(true) {
                ...
                if(pageKey) {
                    response = await axios.get(
                        `https://eth-mainnet.g.alchemy.com/v2/5gLsN5LmEaadZhjV5fGwt1wDs-7inufk/getNFTs?owner=${wallet}&withMetadata=true&pageKey=${pageKey}`
                    );
                    data = response.data;
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 mx-auto">
                    {
                        ...
                        nfts.map((nft, i) => {
                            return (
                                <div key={i} className="flex flex-col justify-center items-center mx-4 rounded-xl shadow-md w-full h-[22rem] mb-8 transform hover:shadow-lg cursor-pointer hover:-translate-y-2 transition duration-500">
                                   <img
                                ...
                                />
                                    ...
                                    <p className="w-full text-base font-medium mt-2 ml-6 ">{nft?.title}</p>
                                </div>
                            )
                        }) 
                    }
